...
namespace norb {
    ...
    template <typename T>
    concept OstreamableWithPrecision =
        requires(T a, std::ostream &os, int precision) {
            {
                os << std::setprecision(precision) << a
    ...
    template <typename G>
    concept IsGraph = requires(G g, const G cg, typename G::node_t u_node,
                               typename G::node_t v_node, size_t n_size) {
        typename G::node_t;
        ...
        requires requires {
            typename std::decay_t<
                decltype(cg.get_edge(u_node, v_node).value())>;
        } &&
        requires(const std::decay_t<
                decltype(cg.get_edge(u_node, v_node).value())>
                    &edge_payload) {
        ...
        {
            cg.get_edges_from(u_node)
        } -> std::ranges::range;  // Check it returns a range
        requires requires(
            const decltype(cg.get_edges_from(u_node)) &edges_range) {
            ...
            requires requires(
                const std::ranges::range_value_t<decltype(edges_range)>
                    &edge_item) {
        ...
        requires requires(const decltype(cg.get_all_edges()) &all_edges_range) {
            ...
            requires requires(
                const std::ranges::range_value_t<decltype(all_edges_range)>
                    &edge_item) {
                { edge_item.from } -> std::same_as<typename G::node_t>;
                { edge_item.to } -> std::same_as<typename G::node_t>;
